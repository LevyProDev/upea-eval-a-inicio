...
import { AdminPersonalData } from "./AdminPersonalDataStep";
import { AdminDocuments } from "./AdminDocumentUploadStep";
...
interface AdminSummaryStepProps {
...
const AdminSummaryStep = ({
  personalData,
  documents,
  email,
  userId,
  onSuccess,
}: AdminSummaryStepProps) => {
  ...
  const handleFinalize = async () => {
    ...
    try {
      const { error } = await supabase.from("admin_profiles").insert({
        ...
        email: email,
        administrative_position: personalData.administrativePosition,
  ...
  const summaryItems = [
    ...
    { icon: Briefcase, label: "Cargo", value: personalData.administrativePosition },
    { icon: Building2, label: "Área/Decanato", value: personalData.department },
...
export default AdminSummaryStep;
