...
import { DirectorPersonalData } from "./DirectorPersonalDataStep";
import { DirectorDocuments } from "./DirectorDocumentUploadStep";
...
interface DirectorSummaryStepProps {
  personalData: DirectorPersonalData;
...
const DirectorSummaryStep = ({
  personalData,
  documents,
  email,
  userId,
  onSuccess,
}: DirectorSummaryStepProps) => {
  ...
  useEffect(() => {
    if (personalData.careerId) {
      fetchCareerName();
    }
  }, [personalData.careerId]);

  const fetchCareerName = async () => {
    const { data } = await supabase
      .from("careers")
      .select("name")
      .eq("id", personalData.careerId)
      .single();
  ...
  const handleFinalize = async () => {
    ...
    try {
      const { error } = await supabase.from("director_profiles").insert({
        user_id: userId,
        first_name: personalData.firstName,
        last_name: personalData.lastName,
        email: email,
        career_id: personalData.careerId || null,
  ...
  const summaryItems = [
    ...
    { icon: GraduationCap, label: "Carrera", value: careerName || "No especificada" },
    { icon: Building2, label: "Facultad", value: personalData.faculty },
    { icon: Briefcase, label: "Cargo", value: personalData.position },
    { icon: Phone, label: "Teléfono", value: personalData.phoneNumber || "No especificado" },
    { icon: CreditCard, label: "Documento", value: `${personalData.documentType} - ${personalData.documentNumber}` },
  ];
...
export default DirectorSummaryStep;
